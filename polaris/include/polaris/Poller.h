/**
 * @file Poller.h
 * @brief PollPoller 和 EPollPoller 的基类，采用“电平触发(LT, Level
 * Trigger)”语意 LT模式，是 epoll 的默认工作模式，这种情况下 epoll
 * 相当于一个效率较高的 poll
 *      -
 * 读事件触发后，可以按需收取想要的字节数，不用把本次接收到的数据收取干净（即不用循环到
 * recv 或者 read 函数返回 -1，错误码为 EWOULDBLOCK 或 EAGAIN）
 *      - 不需要写事件一定要及时移除，避免不必要的触发，浪费 CPU 资源
 *      - 可以自由决定每次收取多少字节（对于普通
 * socket）或何时接收连接（对于侦听 socket），但是可能会导致多次触发
 *
 ...
 *      - 必须每次都要将数据收完（对于普通 socket）或必须理解调用 accept
 * 接收连接（对于侦听socket），其优点是触发次数少
 *
